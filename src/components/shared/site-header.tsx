...
const productItems: MenuItem[] = [
  { label: "Como funciona", href: "/escalas-de-trabalho/#conteudo" },
  { label: "Tipos de escala", href: "/escalas-de-trabalho/#tipos-de-escala" },
  { label: "Vantagens", href: "/escalas-de-trabalho/#recursos" },
];

const aboutItems: MenuItem[] = [
  { label: "Nossa história", href: "/quem-somos/#conteudo" },
  { label: "Público-alvo e propósito", href: "/quem-somos/#publico-alvo" },
];

function Chevron() {
  return (
    <svg className="site-header-chevron" viewBox="0 0 12 12" aria-hidden="true">
...
function ArrowIcon() {
  return (
    <svg className="site-header-item-arrow" viewBox="0 0 12 12" aria-hidden="true">
      <path d="M2.5 9.5 9 3" />
      <path d="M4.5 3H9v4.5" />
    </svg>
  );
...
function Dropdown({ label, href, items, active }: { label: string; href: string; items: MenuItem[]; active: boolean }) {
  ...
  return (
    <div ref={dropdownRef} className={`site-header-dropdown${open ? " is-open" : ""}`}>
      <div className="site-header-dropdown-trigger">
        <Link href={href} className={`site-header-link${active ? " is-active" : ""}`}>{label}</Link>
        <button type="button" className={`site-header-link site-header-dropdown-toggle${active || open ? " is-active" : ""}`} aria-label={`Abrir submenu de ${label}`} aria-haspopup="menu" aria-expanded={open} onFocus={() => setOpen(true)} onClick={() => setOpen((value) => !value)} onKeyDown={(event) => { if (event.key === "Escape") setOpen(false); }}>
          <Chevron />
        </button>
      </div>
      <div className="site-header-submenu">
        <div className="site-header-submenu-card" role="menu" aria-hidden={!open}>
          {items.map((item) => <Link key={item.href} href={item.href} role="menuitem" tabIndex={open ? 0 : -1} onClick={() => setOpen(false)}>{item.label}<ArrowIcon /></Link>)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <header className={`site-header${mobileOpen ? " is-mobile-open" : ""}`}>
      <div className="site-header-left">
        <Link href="/" className="site-header-brand" aria-label="PontoVit, início" onClick={() => setMobileOpen(false)}>
          <span className="site-header-logo-window"><img src="/assets/product/pontovit-logo.png" alt="PontoVit" /></span>
        </Link>
      ...
      <div className="site-header-mobile-panel">
        ...
        <details open={productActive}>
          <summary><Link href="/escalas-de-trabalho/" onClick={() => setMobileOpen(false)}>Escalas de Trabalho</Link><Chevron /></summary>
          <div>{productItems.map((item) => <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}>{item.label}</Link>)}</div>
        </details>
        <details open={aboutActive}>

...
export default function CategoriesPage() {
  ...
  return <main className="pv-admin-subpage"><header className="pv-admin-subpage-header"><div><Link className="pv-admin-back" href="/admin/posts">← Voltar para posts</Link><p className="pv-admin-eyebrow">Conteúdo</p><h1>Categorias</h1><span>Organize os assuntos do blog com slugs estáveis para SEO.</span></div><button className="pv-admin-primary-action" onClick={() => { setError(""); setOpen(true); }}>Nova categoria <b>↗</b></button></header>{message ? <p className="pv-admin-success">{message}</p> : null}{error ? <p className="pv-admin-error">{error}</p> : null}<div className="pv-admin-category-list">{items.map((item) => <div key={item.id}><span><strong>{item.name}</strong><small>/{item.slug}</small></span><button onClick={() => remove(item.id)}>Excluir</button></div>)}</div><AdminModal open={open} title="Nova categoria" description="Crie uma categoria para organizar os posts do blog." onClose={() => setOpen(false)}><form className="pv-admin-inline-form" onSubmit={create}><label className="pv-admin-modal-field">Nome da categoria<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Ex.: Gestão de escalas" required /></label><button className="pv-admin-primary-action">Criar categoria <b>↗</b></button></form></AdminModal></main>;
}
